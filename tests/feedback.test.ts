...
describe('student-facing feedback boundaries',()=>{
 ...
 it('keeps one content guide and every evidence-backed spelling card',()=>{const f=structuredClone(example.feedback);f.cards=[...Array.from({length:3},(_,i)=>({...f.cards[0],id:'c'+i})),...Array.from({length:9},(_,i)=>({...f.cards[1],id:'s'+i}))];const r=normalizeFeedback(feedbackSchema.parse(f),example.ocr);assert.equal(r.cards.filter(c=>c.kind==='content').length,1);assert.equal(r.cards.filter(c=>c.kind==='spelling').length,9)});
 ...
 it('preserves every missing criterion with only one detailed guide',()=>{const f=structuredClone(example.feedback);f.criteria=f.criteria.map(c=>({...c,status:'missing',evidenceIds:[]}));const r=normalizeFeedback(f,example.ocr);assert.equal(r.criteria.filter(c=>c.status==='missing').length,5);assert.equal(r.cards.filter(c=>c.kind==='content').length,1)});
